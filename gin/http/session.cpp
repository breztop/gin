...
void HTTPSession::SetHandler(Handler handler) { handler_ = std::move(handler); }
...
void HTTPSession::ProcessRequest() {
    ...
    if (result.handler) {
        ...
        std::string alt_path = router_->FindAlternativePath(ctx_->request.path);
        if (!alt_path.empty()) {
            std::string redirect_url = alt_path;
            if (!ctx_->request.query_string.empty()) {
                redirect_url += "?" + ctx_->request.query_string;
            }
            LOG_DEBUG("Redirecting {} -> {}", ctx_->request.path, alt_path);
            ctx_->Redirect(301, redirect_url);
        } else {
            // Check for NoRoute handler
            auto no_route = router_->GetNoRouteHandler();
            if (no_route) {
