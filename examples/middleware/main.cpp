...
int main() {
    ...
    engine.Get("/ping", [](gin::Context& ctx) {
    ...
    api.Use(gin::CORS());
    {
        api.Get("/data", [](gin::Context& ctx) {
            ctx.JSON(200, {{"data", {"item1", "item2", "item3"}}});
        });

        api.Post("/data", [](gin::Context& ctx) {
            nlohmann::json body;
    ...
    // 测试 panic 恢复
    engine.Get("/panic", [](gin::Context& ctx) {
        throw std::runtime_error("This is a test panic!");
    });

    // 测试自定义错误处理
    engine.Get("/error", [](gin::Context& ctx) {
