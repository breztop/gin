...
int main() {
    ...
    // 基本查询参数
    engine.Get("/search", [](gin::Context& ctx) {
        auto q = ctx.Query("q");
    ...
    // 多值查询参数
    engine.Get("/filter", [](gin::Context& ctx) {
    ...
    // 路径参数
    engine.Get("/users/:id", [](gin::Context& ctx) {
        auto id = ctx.Param("id");
        ctx.JSON(200, {{"user_id", id}});
    });

    // 多个路径参数
    engine.Get("/users/:user_id/posts/:post_id", [](gin::Context& ctx) {
    ...
    // 通配符路径
    engine.Get("/files/*path", [](gin::Context& ctx) {
        auto path = ctx.Param("path");
        ctx.JSON(200, {{"file_path", path}});
    });

    // 组合使用
    engine.Get("/api/:version/users", [](gin::Context& ctx) {
